...
"""
    ...
    def test_prepare_target_clone_runs_full_offline_metadata_restamp(self) -> None:
        target = self.root.parent / "normalization-target"
        codex_rs = target / "codex-rs"
        member = codex_rs / "member"
...
""",
        ...
        (member / "src/lib.rs").write_text("pub fn example() {}\n", encoding="utf-8")
...
""",
        ...
        subprocess.run(
            [
                "git",
                "-c",
                "user.name=Fork Fleet Test",
                "-c",
                "user.email=fork-fleet-test@localhost",
                "commit",
                "--quiet",
                "-m",
                "stale lock fixture",
        ...
        capture_parent = self.root.parent / "captures"
        capture_parent.mkdir()
        environment = {**os.environ, "CARGO_NET_OFFLINE": "true"}
        production_argv = []
        real_run_capped_process = VALIDATION.run_capped_process

        def record_and_run(argv, *args, **kwargs):
            production_argv.append(argv)
            return real_run_capped_process(argv, *args, **kwargs)

        with (
            mock.patch.object(
                VALIDATION, "validation_temp_parent", return_value=capture_parent
            ),
            mock.patch.object(
                VALIDATION,
                "run_capped_process",
                side_effect=record_and_run,
            ),
        ):
            prepared = VALIDATION.prepare_target_clone(target, environment, target_sha)

        self.assertEqual(
            production_argv,
            [("cargo", "metadata", "--offline", "--format-version", "1")],
        )
        self.assertEqual(prepared[2], " M codex-rs/Cargo.lock\n")
        self.assertEqual(
...
""",
        ...
        self.assertEqual(
            subprocess.check_output(
                ["git", "status", "--short", "-uall"], cwd=target, text=True
            ),
