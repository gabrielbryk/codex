...
- `thread/unsubscribe` — unsubscribe this connection from thread turn/item events. If this was the last subscriber, the server keeps the thread loaded and unloads it only after it has had no subscribers and no thread activity for 5 minutes, runs `SessionEnd` hooks, then emits `thread/closed`.
- Threads created internally by Codex are implicitly attached to every initialized connection so their events and server requests can be delivered. An implicit attachment does not count as a subscriber for the 5-minute idle unload. `thread/start`, `thread/resume`, and `thread/fork` establish retaining subscriptions; resuming an implicitly attached thread upgrades that connection.
...
If this was the last subscriber, the server does not unload the thread immediately. It unloads the thread after the thread has had no subscribers and no thread activity for 5 minutes, runs `SessionEnd` hooks, then emits `thread/closed` and a `thread/status/changed` transition to `notLoaded`.
