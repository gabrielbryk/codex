...
/// Identity of this process's host-managed rolling generation, or `None` for
/// ordinary stdio and unmanaged app-server processes. `codex-app-server-rollout`
/// reads `serverIdentity.capabilities`/`.generation`/`.sourceSha`/`.protocolRevision`
/// from the v1 `initialize` response -- keep these camelCase wire names stable.
