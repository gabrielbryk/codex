...
impl ChatWidget {
    ...
    pub(super) fn status_line_command_input(&self) -> Option<StatusLineCommandInput> {
        ...
        // Repository host/owner/name metadata is not available from
        // `StatusLineGitSummary` in this target; the wire payload omits it.
        let repo: Option<StatusLineCommandRepository> = None;
