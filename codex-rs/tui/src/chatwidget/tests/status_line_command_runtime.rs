...
async fn same_cwd_thread_reset_rejects_old_dependency_results() {
    ...
    chat.set_status_line_git_summary(stale_cwd, StatusLineGitSummary::default());
    ...
    assert_eq!(
        ...
        Some(
            crate::status_line_command::wire::StatusLineCommandCurrentUsage {
                input_tokens: 175_000,
                output_tokens: 12_000,
                // This target's `TokenUsage` does not track write-cache tokens
                // separately, so the wire payload always reports zero here.
...
#[tokio::test]
async fn command_input_omits_repository_metadata_without_a_pull_request() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    install_command(&mut chat, vec!["formatter".to_string()]);
    ...
    let input = chat.status_line_command_input().expect("command input");
    // `StatusLineGitSummary` does not carry host/owner/repo-name metadata on this
    // target, so the wire payload never populates `workspace.repo`.
    assert_eq!(input.workspace.repo, None);
