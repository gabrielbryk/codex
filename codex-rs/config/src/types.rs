...
pub struct TuiStatusLineCommand {
    /// Absolute executable path followed by literal arguments. Codex does not invoke a shell or
    /// search `PATH`.
...
pub struct Tui {
    ...
    ///
    /// When a different config layer sets `status_line`, the higher-precedence setting wins. Both
    /// settings in the same layer are invalid.
    #[serde(default)]
