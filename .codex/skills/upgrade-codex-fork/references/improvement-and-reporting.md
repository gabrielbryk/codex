...
Never extrapolate a full commit SHA from abbreviated terminal output. Record candidate and lane SHAs
only from `git rev-parse HEAD` or an exact immutable tool response, and verify the object exists with
`git cat-file -e <sha>^{commit}` before reporting it or passing it to finalize.
